...
struct InterceptorMessage { action: String, bytes: Option<u64> }

#[derive(Serialize, Debug)]
struct DaemonResponse { status: String }

#[tokio::main]
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    let socket_path = "/tmp/nyx.sock";
    if fs::metadata(socket_path).is_ok() { fs::remove_file(socket_path)?; }

    let listener = UnixListener::bind(socket_path)?;
    println!("🚀 [ASYNC IPC] Scheduler Brain online! Listening on {}...", socket_path);
...
async fn handle_client(mut stream: UnixStream) {
    ...
    if let Ok(Ok(size)) = timeout(Duration::from_secs(2), stream.read(&mut buffer)).await {
        ...
        if let Ok(req) = serde_json::from_str::<InterceptorMessage>(&raw_msg) {
            println!("📥 [REQUEST] Action: {}, Bytes: {:?}", req.action, req.bytes);
            
            let res = serde_json::to_string(&DaemonResponse { status: "GO".into() }).unwrap();
